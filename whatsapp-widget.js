window.whatsapp = class {
	constructor({
		...
		this.wAgents      = agents,
		this.wTime        = time
		if (!this.isEnabled(this.wTime)) return
		this.render()
	}
	getAgent({cta, hours, name, phone}) {
	...
	getHeader() {
		return `<header class="wa-w_h">
			<span class="wa-w_h_t">${this.wTitle}</span>
			<span class="wa-w_h_s">
	...
	render() {
		...
		let a = "";
		for (const s of this.wAgents) a += this.getAgent(s);
		document.body.insertAdjacentHTML('beforeend',
			`${this.getStyles()}
			<div class="wa-w">
				${this.getHeader()}
				<section class="wa-w_a">${a}</section>
				<button class="wa-w_b" title="${this.wTitle}"></button>
			</div>`),
